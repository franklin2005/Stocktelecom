...
<div class="row g-4">
    <div class="col-12 col-xl-7">
        <div class="card shadow-sm h-100">
            <div class="card-body">
                <h5 class="card-title mb-3">Super administradores</h5>

                @if ($superAdmins->isEmpty())
                    <p class="text-muted mb-0">No hay super administradores registrados.</p>
                @else
                                        ...
                                        <td class="text-end">
                                            <div class="d-flex justify-content-end gap-2">
                                                @if ($canViewMovements && $canManageSuperAdmins)
                                                    <a href="{{ route('admin.personnel.movements', $superAdmin) }}" class="btn btn-sm btn-outline-secondary">
                                                        Ver movimientos
                                                    </a>
                                                @endif
                                                @if ($canManageSuperAdmins)
                                                    <a href="{{ route('admin.personnel', ['tab' => 'super_admins', 'edit_super_admin' => $superAdmin->id]) }}" class="btn btn-sm btn-outline-primary">
                                                        Editar
                                                    </a>
                                                    <form method="POST" class="d-inline" action="{{ route('admin.staff.destroy', ['staff' => $superAdmin->id, 'tab' => 'super_admins']) }}" onsubmit="return confirm('Seguro que deseas eliminar este super administrador?');">
                                                        @csrf
                                                        @method('DELETE')
    ...
    <div class="col-12 col-xl-5">
        @if ($canManageSuperAdmins)
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    <h5 class="card-title mb-3">Crear super administrador</h5>
                    <form method="POST" action="{{ route('admin.staff.store', ['tab' => 'super_admins']) }}">
                        ...
                        <div class="mb-3">
                            <label for="super_admin_email" class="form-label">Correo electronico</label>
                        ...
                        <div class="mb-3">
                            <label for="super_admin_password" class="form-label">Contrasena</label>
                        ...
                        <div class="mb-3">
                            <label for="super_admin_password_confirmation" class="form-label">Confirmar contrasena</label>
                            <input type="password" id="super_admin_password_confirmation" name="password_confirmation" class="form-control" required>
                        </div>
                        <button type="submit" class="btn btn-primary w-100">Crear super administrador</button>
                    </form>
                </div>
            </div>
                ...
                <div class="card shadow-sm">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="card-title mb-0">Editar super administrador</h5>
                            <a href="{{ route('admin.personnel', ['tab' => 'super_admins']) }}" class="btn btn-sm btn-outline-secondary">Cancelar</a>
                        </div>
                        <form method="POST" action="{{ route('admin.staff.update', ['staff' => $editingSuperAdmin->id, 'tab' => 'super_admins']) }}">
                            ...
                            <div class="mb-3">
                                <label for="edit_super_admin_email" class="form-label">Correo electronico</label>
                            ...
                            <div class="mb-3">
                                <label for="edit_super_admin_password" class="form-label">Contrasena (opcional)</label>
                                <input type="password" id="edit_super_admin_password" name="password" class="form-control @error('password', 'updateStaff') is-invalid @enderror" placeholder="Deja vacio para mantener la actual">
                                @error('password', 'updateStaff')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-3">
                                <label for="edit_super_admin_password_confirmation" class="form-label">Confirmar contrasena</label>
                                <input type="password" id="edit_super_admin_password_confirmation" name="password_confirmation" class="form-control" placeholder="Requerido solo si cambias la contrasena">
                            </div>
                            <div class="mb-3">
                                ...
                                <select id="edit_super_admin_role" name="role" class="form-select @error('role', 'updateStaff') is-invalid @enderror">
                                    <option value="super_admin" @selected(old('role', $editingSuperAdmin->role) === 'super_admin')>Super Administrador</option>
                            ...
                            <button type="submit" class="btn btn-success w-100">Actualizar super administrador</button>
                        </form>
                    </div>
                </div>
            @endif
