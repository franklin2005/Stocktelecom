                                        ...
                                        <td class="text-end">
                                            <div class="d-flex justify-content-end gap-2">
                                                ...
                                                @if ($canManageTechnicians)
                                                    ...
                                                    <form method="POST" class="d-inline" action="{{ route('admin.technicians.destroy', ['technician' => $technician->id, 'tab' => 'technicians']) }}" onsubmit="return confirm('¿Seguro que deseas eliminar este técnico?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-outline-danger" {{ $hasStock ? 'disabled' : '' }} @if ($hasStock) title="Vacía el stock antes de eliminar este técnico." @endif>
                                                            Eliminar
                                                        </button>
                                                    </form>
                                                    @if ($hasStock)
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('admin.technicians.store', ['tab' => 'technicians']) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="create_tech_code" name="tech_code" class="form-control @error('tech_code', 'createTechnician') is-invalid @enderror" value="{{ old('tech_code') }}" placeholder="Se generará automáticamente si lo dejas vacío">
                            @error('tech_code', 'createTechnician')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <button type="submit" class="btn btn-primary w-100">Crear técnico</button>
                    </form>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('admin.technicians.update', ['technician' => $editingTechnician->id, 'tab' => 'technicians']) }}">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="password" id="edit_password" name="password" class="form-control @error('password', 'updateTechnician') is-invalid @enderror" placeholder="Deja vacío para mantener la actual">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="password" id="edit_password_confirmation" name="password_confirmation" class="form-control" placeholder="Requerido solo si cambias la contraseña">
                            </div>
                            <div class="mb-3">
                            ...
                            <button type="submit" class="btn btn-success w-100">Actualizar técnico</button>
                        </form>
                    </div>
                </div>
            @endif
