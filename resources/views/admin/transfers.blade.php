...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h2 class="mb-1">Transferencias desde almacen</h2>
            <p class="text-muted mb-0">Selecciona materiales del almacen principal y genera solicitudes para los tecnicos.</p>
        </div>
    </div>
    ...
    <div class="row g-4 mb-4">
        <div class="col-12 col-xxl-8">
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    ...
                    <p class="text-muted small mb-3">Selecciona la cantidad que deseas enviar a un tecnico.</p>

                    @if ($nonSerializedInventory->isEmpty())
                        <p class="text-muted mb-0">No hay materiales no serializados disponibles en el almacen.</p>
                    @else
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    ...
                    <p class="text-muted small mb-3">Marca los numeros de serie que deseas enviar.</p>

                    @if ($availableSerials->isEmpty())
                        <p class="text-muted mb-0">No hay numeros de serie disponibles en el almacen.</p>
                    @else
                        @php
                                    ...
                                    <div class="accordion-item">
                                        <h2 class="accordion-header" id="heading-{{ $accordionId }}">
                                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $accordionId }}" aria-expanded="false" aria-controls="collapse-{{ $accordionId }}">
                                                ...
                                                @if ($inCartCount > 0)
                                                    <span class="badge bg-info ms-2">{{ $inCartCount }} en lista</span>
                                                @endif
                                            </button>
                                        </h2>
        ...
        <div class="col-12 col-xxl-4">
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    <h5 class="card-title mb-3">Seleccionar tecnico</h5>
                    ...
                    <form method="POST" action="{{ route('admin.transfers.send') }}" id="admin-transfer-form" data-total-units="{{ $cartSummary['total_units'] }}">
                                        ...
                                        <label class="list-group-item list-group-item-action d-flex align-items-center gap-2">
                                            ...
                                            <span>
                                                ...
                                                @if ($technician->tech_code)
                                                    <small class="text-muted">({{ $technician->tech_code }})</small>
                                                @endif
                                            </span>
                                        </label>
                                    @endforeach
                        ...
                        <button type="submit" class="btn btn-primary w-100" {{ empty($cartItems) || $technicians->isEmpty() ? 'disabled' : '' }}>
            ...
            <div class="card shadow-sm">
                <div class="card-body">
                    ...
                    <ul class="list-unstyled mb-0">
                        <li class="mb-2"><strong>Almacen:</strong> {{ $warehouse->name }}</li>
